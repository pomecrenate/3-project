	...
	public boolean modify(ModifyCompanyRequest modifyRequest) {
		...
		try {
			...
			// ログインIDが作成者IDと異なる場合は例外
	         if (!PermissionChecker.canModify(modifyRequest.getCompanyId(), companyDao.selectByCode(conn, modifyRequest.getCompanyCode()))) {
			...
			Company company = new Company(companyCode, businessType, businessItem, password, 
					companyName, ceoName, businessNumber, 
					corporateNumber, establishmentDate, website, address, phoneNumber, faxNumber);
